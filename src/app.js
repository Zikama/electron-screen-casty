...
const recorderOnDataAvailable = (event) => {
    if (event.data && event.data.size > 0) {
        ...
        reader.addEventListener('loadend', _raw => {
            const results = new Int8Array(reader.result /* .split(',')[1] */ );
            let chunkData = results
            chunkData = Buffer.from(new Int8Array(chunkData)) /* .toString('base64') */
            fs.appendFileSync('./src/assets/chuncks/' + /* Date.now()  */ 'chunk' + '-nemie-made.webm', chunkData)
        })
